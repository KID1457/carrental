...
<link href="<%=basePath%>member/css/bootstrap.min.css" rel="stylesheet">
<link href="<%=basePath%>member/css/Global.css" rel="stylesheet">
<script type="text/javascript"
	src="<%=basePath%>member/script/zDialog.js"></script>
<script type="text/javascript" src="<%=basePath%>member/script/zDrag.js"></script>
<script type="text/javascript">
	...
	function chinaChange(province, city) {
		...
		city.options[0].text = "select city";
...
<title>Frequent car renters</title>
</head>
<body>
	<div id="content">
		<div class="content-bd">
			<div class="col-main">
				<div class="renter">
					<h2>Frequent car renters</h2>
					<div class="mod renter-list">
						<ul class="driver-list">
							<li class="row-title"><span class="row-first-cell cell-name">name</span>
								<span class="cell-phone">phone</span> <span class="cell-idtype">document
									type</span> <span class="cell-idnumber">card num</span> <span
								class="cell-passport">address</span> <span class="cell-operate">options</span>
							</li>
							<%
							...
							<li class="" data-driver-id="<%=fc.getFrequentId()%>"
								...
								data-month="1"><span class="row-first-cell cell-name"><%=fc.getFrequentName()%></span>
								<span class="cell-phone"><%=fc.getFrequentPhone()%></span> <span
								class="cell-idtype"><%=fc.getIdType()%></span> <span
								class="cell-idnumber"><%=fc.getIdentity()%></span> <span
								class="cell-passport"><%=fc.getFrequentProvince()%> <%=fc.getFrequentCity()%>
									<%=fc.getFrequentAddresss()%></span> <span class="cell-operate">
									<a class="delete1"
									href="<%=basePath%>servlet/MemberServlet?method=delFrequent&id=<%=fc.getFrequentId()%>"
							...
							<li class="ck" data-driver-id="<%=fc.getFrequentId()%>"
								...
								class="cell-passport"><%=fc.getFrequentProvince()%> <%=fc.getFrequentCity()%>
									<%=fc.getFrequentAddresss()%></span> <span class="cell-operate">
									<a class="delete1"
									href="<%=basePath%>servlet/MemberServlet?method=delFrequent&id=<%=fc.getFrequentId()%>">delete</a>
						...
						<p class="ui-checkbox">
							<label> <input type="checkbox" id="checkboxAgree"
								checked="checked"> Agree
							</label> <a href="javascript:void(0)" class="btn" data-toggle="modal"
								data-target="#agreementModal">《Add common car rental notice》</a>
						</p>
						<div class="operate-panel">
							<a id="btnAddDriver" href="javascript:void(0)"><img
								src="<%=basePath%>member/images/changyongzucheren.png"> </a>
						</div>
					</div>
			...
			<div class="modal-dialog vertical-align-center">
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title">Notice to common car renters</h4>
					</div>
					<div class="modal-body">
						<p>Dear customer：</p>
						<p style="text-indent: 2em;">If you need to add a common car
							renter (hereinafter referred to as the third party), you must
							ensure that the third party's behavior ability is effective, all
							identity information is true and effective, has legal driving
							qualification, etc.; you will bear unlimited joint and several
							liability for the third party's operation in your account (such
							as cancellation, change of basic information, order placement,
							etc.); you will perform the signed services for the third party
							and our company The rights and obligations stipulated in the
							agreement and all car rental documents shall bear unlimited joint
							and several liabilities, including but not limited to the return
							of vehicles, payment of rental fees, payment of fines for
							violation of regulations, and bearing the liability for traffic
							accidents. If you still need to add common car renters, it will
							be deemed that all the above agreements are recognized。</p>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-primary btn-agree">
							Terms read and agreed</button>
					</div>
				</div>
				<!-- /.modal-content -->
			</div>
