...
Map<String,Object> frequentMap =  contactsDAO.getFrequentByFrequentId(Integer.parseInt(orderMap.get("frequentId").toString() ));
 %>
							<span> | </span>frequentName：<%=frequentMap.get("frequentName") %>
							<span> | </span>pay type：online <span>|</span>orderState：<%=orderMap.get("orderState") %>
						</p>
					</div>
						...
						<div class="ui-bfc-bd car-info-detail">
							...
							<table width="100%">
								<tbody>
									...
									<tr>
										<td>Picking time：</td>
										<td class="cell-cnt"><%=beginTime.substring(0,19) %></td>
										<td>Return time：</td>
										<td class="cell-cnt"><%=endTime.substring(0,19) %></td>
									</tr>
								...
								<div class="price-info-item-hd">
									<h3>
										Vehicle rental fee and store service fee <span
											class="insurance-count">(Prepaid price)</span>
									</h3>
									<p>
...
if(shangMenQuCheAddress.length()!=0){
 %>
							...
							<div class="price-info-total">
								<span class="txt">sum：</span> <sup class="ui-rmb"> ¥ </sup>
					...
					<div class="order-operate">
						...
						<a id="btn-cancel" class="ui-btn ui-btn-white-l btn-cancel"
							...
							onclick='javascript:return del()'><span>cancal order</span> </a>
					</div>
					<%}else if(orderMap.get("orderState").equals("Waiting for departure")){ %>
					<div class="order-operate">
						<a id="btn-cancel" class="ui-btn ui-btn-white-l btn-cancel"
							href="<%=basePath %>servlet/OrderServlet?method=outCar&orderId=<%=orderMap.get("orderId") %>&carId=<%=carMap.get("carId") %>"><span>do
								departure</span> </a> <a id="btn-cancel"
							class="ui-btn ui-btn-white-l btn-cancel"
							href="<%=basePath %>servlet/OrderServlet?method=cancelOrder&orderId=<%=orderMap.get("orderId") %>&carId=<%=carMap.get("carId") %>"
							onclick='javascript:return del()'><span>cancel order</span> </a>
					</div>
					<%}%>
				</div>
			</div>
	...
	$('#prepayCountDown')
			.countdown(
					...
					function(event) {
						$(this)
								.html(
										event.strftime('Time to final payment:' +
										//'<span>%D</span> day ' +
